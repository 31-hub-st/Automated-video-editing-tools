...
- 制作台只能选择小说库已绑定且有效的平台/口令；员工可自由编辑本批口令卡和结尾引导的完整模板，但必须把 `{platform}`、`{code}` 各保留一次。服务端保存与排队时重新核对真实绑定并替换、冻结平台和口令，不能由浏览器伪造。
...
& '.\scripts\build_exe.ps1' `
  ...
  -OutputDirectory 'D:\StoryForgeRelease\1.0.8\dist' `
  -WorkDirectory 'D:\StoryForgeRelease\1.0.8\work' `
