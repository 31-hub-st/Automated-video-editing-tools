...
当前正式基线为 `v1.0.6`，当前源码候选版本为 `v1.0.7`。在 [v1.0.7 发布交付报告](docs/V1.0.7_RELEASE.md) 的测试、哈希和远程门禁 TODO 全部由真实结果替换前，不得把候选源码或本地目录当成已交付程序。
...
`v1.0.7` 的发布阻断项和待填写证据见 [v1.0.7 发布交付报告](docs/V1.0.7_RELEASE.md)；`v1.0.6` 的正式证据保留在 [v1.0.6 发布交付报告](docs/V1.0.6_RELEASE.md)。
...
- 支持本地 Kokoro、Edge TTS 和 Deepgram；按当前供应商、小说语言和本机组件列出全部真实可用女声，可搜索、按风格筛选、收藏、个人隐藏或由管理员团队停用。
- 音色目录与试听分离，点击某个真实 Voice ID 时才生成单条试听；语速试听使用独立请求、缓存与 latest-request-wins，不再重新生成候选列表。
- 支持舒适 220、推荐 240、快速 260、极快 280 WPM，以及 200–280 WPM 自定义试听。
- 支持整句字幕、单词逐个出现、逐词高亮、稳定字幕、口令卡、安全区、字体/颜色/描边/位置自定义。
- 支持简介卡与口令卡各自独立开关；开始位置可按正文秒数或正文进度，显示范围可按秒数、正文比例或直到正文结束，并在配音完成后冻结真实生效区间。
- 制作台只能选择小说库已绑定且有效的平台/口令；平台名、口令和模板解析由服务端权威冻结，员工只能编辑不含平台或口令的结尾前后文。
...
& '.\scripts\build_exe.ps1' `
  ...
  -OutputDirectory 'D:\StoryForgeRelease\1.0.7\dist' `
  -WorkDirectory 'D:\StoryForgeRelease\1.0.7\work' `
