# StoryForge v1.0.1 局域网软件更新

> 适用源码版本：StoryForge Studio `v1.0.1`。本次正式包、哈希与验收结果见 [v1.0.1 正式版交付报告](V1.0.1_RELEASE.md)。

`v1.0.1` 将核心程序和可变数据分开：程序可安装在任意合格的本地目录，设置、缓存、更新状态和语言组件位于用户选择的 `<StoryForgeData>`。Hub 可以向已登记制作电脑发布完整核心程序 ZIP；客户端自动轮询、在空闲时下载并校验，并在安全重启点安装。它不是运行中热替换，正在制作的任务不会被覆盖或打断。

旧版迁移兼容：任何 `0.x` 制作电脑都应先结束任务并完全退出旧桌面端与 Worker，再人工安装 `v1.0.1` 完整包到新的纯英文目录；完成一条真实任务并确认数据无误后，才可清理旧目录。
...
- 每个组件保留当前版和上一版，可一键回退；安装或断电失败不会暴露半解压目录。`v1.0.1` 完整包可内置已验收的日语组件，后续语种通过独立组件包扩展。
...
软件更新协议与生产审批无关。`v1.0.1` 的制作电脑在浏览器即时预览后，直接创建 `job_kind="full"`、`status="queued"` 的完整任务，`preview_required=false`；不会为新批次生成或审批独立 preview。操作可从桌面窗口、Hub 网页或该电脑的本地制作网页发起；浏览器中的媒体调用由当前电脑的本机制作服务执行。更新后应确认 `StoryForge Local Worker` 登录任务仍存在并已随新版程序重启，完整窗口无需常开。
...
- 旧版迁移目录须在 `v1.0.1` 真机验收通过后由管理员决定归档或删除。程序不会自动删除仍有诊断价值的旧数据库、设置、日志、Manifest 或未知文件。
