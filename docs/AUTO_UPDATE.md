...
> 适用源码版本：StoryForge Studio `0.4.8`（稳定版）。当前文件、哈希与验收结果见 [0.4.8 稳定版交付报告](V0.4.8_STABLE_RELEASE.md)；[0.4.7 交付报告](V0.4.7_STABLE_RELEASE.md) 仅为历史记录。0.4.2 → 0.4.3 的一次性受控迁移规则仍保留在下文。

`0.4.8` 将核心程序和可变数据分开：程序可安装在任意合格的本地目录，设置、缓存、更新状态和语言组件位于用户选择的 `<StoryForgeData>`。Hub 可以向已登记制作电脑发布完整核心程序 ZIP；客户端自动轮询、在空闲时下载并校验，并在安全重启点安装。它不是运行中热替换，正在制作的任务不会被覆盖或打断。
...
- 每个组件保留当前版和上一版，可一键回退；安装或断电失败不会暴露半解压目录。`0.4.8` 完整包中的日语仍内置可工作，当前 TTS 执行逻辑不依赖外置组件包。
...
软件更新协议与生产审批无关。`0.4.8` 的制作电脑在浏览器即时预览后，直接创建 `job_kind="full"`、`status="queued"` 的完整任务，`preview_required=false`；不会为新批次生成或审批独立 preview。操作可从桌面窗口、Hub 网页或该电脑的本地制作网页发起；浏览器中的媒体调用由当前电脑的本机制作服务执行。更新后应确认 `StoryForge Local Worker` 登录任务仍存在并已随新版程序重启，完整窗口无需常开。
