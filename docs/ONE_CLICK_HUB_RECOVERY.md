...
1. 在浏览器登录有权读取私有仓库的 GitHub 账号，从最新稳定 Release 下载正式更新 ZIP（文件名为 `StoryForge-<版本>-update.zip`）；不要下载 GitHub 的 Source code/源码 ZIP。
2. 把正式更新 ZIP 完整解压到新电脑的普通文件夹，不要在 ZIP 内直接运行。
...
2. 新电脑准备本地 `D:` 盘，并把正式更新 ZIP 完整解压到新电脑普通文件夹中；不要下载源码 ZIP，也不要在 ZIP 内直接运行。
