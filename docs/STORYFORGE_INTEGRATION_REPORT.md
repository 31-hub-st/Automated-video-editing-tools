# StoryForge Studio v1.0.2 项目接手与功能嫁接说明

本文是 v1.0.2 的权威技术接手文档。旧 0.x 交付报告、临时路径、历史哈希和已经下线的 UI 流程不再构成当前合同。
...
v1.0.2 仍兼容旧设备发送不含 `selection_key` 的声音候选；新客户端会按 Hub 公布的能力字段投影请求，连接未公布清单的旧 Hub 时使用兼容字段集。服务端仍严格拒绝未声明字段，不允许吞掉其他 400 错误。
...
GitHub Release 仅保存正式制品供管理员下载，不等于 Hub 发布。v1.0.2 首次交付默认由管理员手动发给员工；确认员工下班并完成真机冒烟后，再决定是否在 Hub 发布自动更新清单。
...
- `docs/V1.0.2_RELEASE.md`：正式制品与验证证据。
