# StoryForge Studio v1.0.7 项目接手与功能嫁接说明

本文是 v1.0.7 源码候选的权威技术接手文档；当前正式基线为 v1.0.6。旧 0.x 交付报告、临时路径、历史哈希和已经下线的 UI 流程不再构成当前合同。
...
自 v1.0.2 起支持旧设备发送不含 `selection_key` 的声音候选，v1.0.3 与 v1.0.5 候选均已延续该兼容合同，当前 v1.0.7 继续保持；新客户端会按 Hub 公布的能力字段投影请求，连接未公布清单的旧 Hub 时使用兼容字段集。服务端仍严格拒绝未声明字段，不允许吞掉其他 400 错误。v1.0.7 新任务还冻结 `production_contract=2`；旧 Worker 可完成旧任务，但必须升级后才能领取需要混合卡片时间轴和平台权威语义的新任务。
...
GitHub Release 仅保存程序制品供管理员下载，不等于 Hub 发布。v1.0.6 是当前正式基线；v1.0.7 只有在真实完整测试、冻结构建、包体哈希和远程门禁全部通过后才可成为稳定最新版。默认仍由管理员手动发给员工，确认员工下班并完成真机冒烟后，再决定是否在 Hub 发布自动更新清单。
...
- `docs/V1.0.7_RELEASE.md`：当前候选制品的阻断项与待验证证据；完成前不得发布。
- `docs/V1.0.5_RELEASE.md`：上一候选版本的真实制品、门禁与零下载暂缓分发事实。
- `docs/V1.0.4_RELEASE.md`：更早候选版本的历史制品、门禁与暂缓分发事实。
