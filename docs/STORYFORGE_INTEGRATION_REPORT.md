# StoryForge Studio v1.0.5 项目接手与功能嫁接说明

本文是 v1.0.5 的权威技术接手文档。旧 0.x 交付报告、临时路径、历史哈希和已经下线的 UI 流程不再构成当前合同。
...
自 v1.0.2 起支持旧设备发送不含 `selection_key` 的声音候选，v1.0.3 已延续该兼容合同，v1.0.5 继续保持；新客户端会按 Hub 公布的能力字段投影请求，连接未公布清单的旧 Hub 时使用兼容字段集。服务端仍严格拒绝未声明字段，不允许吞掉其他 400 错误。
...
GitHub Release 仅保存程序制品供管理员下载，不等于 Hub 发布。v1.0.2、v1.0.3 和 v1.0.4 均已暂缓分发；其中 v1.0.4 虽完成构建和远程门禁，但旧 Hub 任务主体显示形式会造成同一 Windows 账号被误拒绝，且附件下载数为 0，现为预发布并且不是 `latest`。v1.0.5 只有在真实完整测试、冻结构建、包体哈希和远程门禁全部通过后才可成为稳定最新版；默认仍由管理员手动发给员工，确认员工下班并完成真机冒烟后，再决定是否在 Hub 发布自动更新清单。
...
- `docs/V1.0.5_RELEASE.md`：当前候选制品的阻断项与待验证证据；完成前不得发布。
- `docs/V1.0.4_RELEASE.md`：上一候选版本的历史制品、门禁与暂缓分发事实。
