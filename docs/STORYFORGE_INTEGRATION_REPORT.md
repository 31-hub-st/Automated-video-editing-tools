# StoryForge Studio v1.0.8 项目接手与功能嫁接说明

本文是 `v1.0.8` 正式稳定版的权威技术接手文档；GitHub `Latest` Release、Hub 员工更新通道和当前生产合同均以 `v1.0.8` 为基线。旧版交付报告、临时路径、历史哈希和已经下线的 UI 流程只用于追溯，不再构成当前合同。
...
6. 视觉
   - 顶部口令卡与简介卡均可独立开关，并按正文秒数或正文百分比配置开始与显示范围，不再固定全程展示。
...
- 顶部口令卡和简介卡按冻结的正文时间窗显示；开始位置可用正文秒数/百分比，显示范围可用秒数/正文百分比/直到正文结束，且不得延长最终成片。卡片文本垂直/水平居中并限制在安全区。
...
自 v1.0.2 起支持旧设备发送不含 `selection_key` 的声音候选，v1.0.3 与 v1.0.5 候选均已延续该兼容合同，当前 v1.0.8 继续保持；新客户端会按 Hub 公布的能力字段投影请求，连接未公布清单的旧 Hub 时使用兼容字段集。服务端仍严格拒绝未声明字段，不允许吞掉其他 400 错误。自 v1.0.7 起的新任务冻结 `production_contract=2`；旧 Worker 可完成旧任务，但必须升级后才能领取需要混合卡片时间轴和平台权威语义的新任务。
...
GitHub Release 保存管理员用于新安装与恢复的正式程序制品，Hub 员工更新通道是独立发布步骤。当前 `v1.0.8` 已完成真实完整测试、冻结构建、包体哈希、远程门禁和生产验收，并以同一正式 ZIP 同时作为 GitHub `Latest` 稳定 Release 与 Hub 员工更新。后续版本仍须分别完成 GitHub 发布和 Hub 更新通道发布，不能把其中一个状态当成另一个。
...
这些是后续可维护性优化，不改变 `v1.0.8` 已发布合同，也不是新电脑恢复的前置条件。
...
- `docs/V1.0.8_RELEASE.md`：当前正式稳定制品、哈希、门禁和生产发布证据。
- `docs/V1.0.7_RELEASE.md`：上一候选版本的历史证据与失败门禁。
- `docs/V1.0.5_RELEASE.md`：更早候选版本的真实制品、门禁与零下载暂缓分发事实。
- `docs/V1.0.4_RELEASE.md`：历史制品、门禁与暂缓分发事实。
